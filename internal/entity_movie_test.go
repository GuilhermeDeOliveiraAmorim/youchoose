...
func TestNewMovie(t *testing.T) {
	movie, err := NewMovie("Inception", Nationality{"USA", "🇺🇸"}, []Genre{{Name: "Sci-Fi"}}, []Director{{Name: "Christopher Nolan"}}, []Actor{{Name: "Leonardo DiCaprio"}}, []Writer{{Name: "Christopher Nolan"}}, 2010, "image123")
	if err != nil {
		t.Errorf("Erro ao criar um novo filme válido: %v", err)
	}

	if movie.Title != "Inception" || movie.Nationality.CountryName != "USA" || movie.Genres[0].Name != "Sci-Fi" || movie.Directors[0].Name != "Christopher Nolan" || movie.Actors[0].Name != "Leonardo DiCaprio" || movie.Writers[0].Name != "Christopher Nolan" || movie.ReleaseYear != 2010 || movie.ImageID != "image123" {
		t.Errorf("O filme não foi criado corretamente. Detalhes do filme: %v", movie)
	}

	_, err = NewMovie("", Nationality{"USA", "🇺🇸"}, []Genre{{Name: "Sci-Fi"}}, []Director{{Name: "Christopher Nolan"}}, []Actor{{Name: "Leonardo DiCaprio"}}, []Writer{{Name: "Christopher Nolan"}}, 2010, "image123")
	if err == nil {
...
func TestMovie_AddActors(t *testing.T) {
	movie, _ := NewMovie("Inception", Nationality{"USA", "🇺🇸"}, []Genre{{Name: "Sci-Fi"}}, []Director{{Name: "Christopher Nolan"}}, []Actor{{Name: "Leonardo DiCaprio"}}, []Writer{{Name: "Christopher Nolan"}}, 2010, "image123")
...
func TestMovie_RemoveDirectors(t *testing.T) {
	birthDate, err := NewBirthDate(15, 5, 2010)
	if err != nil {
		fmt.Println(err)
	}
	
	nationality, err := NewNationality("United States", "🇺🇸")
	if err != nil {
		fmt.Println(err)
	}
...
func TestMovie_IncrementVotes(t *testing.T) {
	// Criando um filme inicial
	movie, _ := NewMovie("Inception", Nationality{"USA", "🇺🇸"}, []Genre{{Name: "Sci-Fi"}}, []Director{{Name: "Christopher Nolan"}}, []Actor{{Name: "Leonardo DiCaprio"}}, []Writer{{Name: "Christopher Nolan"}}, 2010, "image123")

	// Incrementando votos
	movie.IncrementVotes()

	// Verificando se os votos foram incrementados corretamente
	if movie.Votes != 1 {
